...
onMounted(() => {
  ...
  hippo.addEventListener("mouseenter", handleMouseEnter);
  hippo.addEventListener("mouseleave", handleMouseLeave);

  onBeforeUnmount(() => {
    hippo.removeEventListener("mouseenter", handleMouseEnter);
    hippo.removeEventListener("mouseleave", handleMouseLeave);
  });
});
</script>
...
<template>
  <div class="flex justify-center">
    <svg
      ...
      viewBox="0 0 640 640"
      class="w-80 h-80 cursor-pointer"
    >
      <!-- 疊加眼睛 -->
      <ellipse id="eyeLeft" cx="456" cy="296" rx="12" ry="12" fill="white" />
      <ellipse id="eyeRight" cx="568" cy="296" rx="12" ry="12" fill="white" />
